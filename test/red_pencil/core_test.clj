...
(facts
  ...
  (fact
    ...
    (let [good {:price (price 200) :on-promotion false}]
      (red-pencil/change-price good (price 250)) => {:price (price 250) :on-promotion false}))

  (facts
    "price reductions activate promotions"
    (let [good {:price (price 100) :on-promotion false}]
      (red-pencil/change-price good (price 95)) => {:price (price 95) :on-promotion true}
      (red-pencil/change-price good (price 70)) => {:price (price 70) :on-promotion true}

      (fact
        "except when the price is reduced less than 5%"
        (red-pencil/change-price good (price 98)) => {:price (price 98) :on-promotion false})

      (fact
        "except when the price is reduced more than 30%"
        (red-pencil/change-price good (price 69)) => {:price (price 69) :on-promotion false}))))
